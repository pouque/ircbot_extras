...
def list.get_by_hash {α : Type} [inhabited α] (s : string) (xs : list α) : α :=
...
def colours :=
  ["nigger", "american nigger", "yellow-cheeked", "asian", "mexican",
   "bashkir", "buryat", "tatarstanian", "white"]
def sizes :=
  ["extremly fat", "abnormally fat", "very fat", "fat", "normal weight",
   "extremly thin", "abnormally thin", "very thin", "thin", "skinny"]
def look :=
  ["very sexy", "sexy", "pretty", "normal looking", "unremarkable",
   "ugly", "freak", "retarded looking"]
def religious :=
...
string.intercalate " " (list.get_by_hash nick <$> specs)
