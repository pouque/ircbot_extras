...
namespace ircbot_external.translate
...
def CorrectTranslateCommand : parser (option string) := do
  Prefix, optional (parsing.Ws >> parsing.FreeWord)

def translate_func (input : irc_text) : io (list irc_text) :=
match input with
...
def translate : bot_function :=
  { name := "retranslator 9000",
    ...
    func := λ t, t >>= translate_func }

end ircbot_external.translate
